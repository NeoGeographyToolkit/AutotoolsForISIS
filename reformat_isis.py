...
def write_makefile_am_from_objs_dir( directory ):
    ...
    headers = glob(P.join(directory,'*/*.h'))
    print('include_HEADERS = ', file=makefile, end='')
    for header in headers:
        relative_header = P.relpath( header, directory )
...
if __name__ == '__main__':
    ...
    # Traverse and copy all files which are not make files
    shutil.copytree( P.join(opt.isisroot,'src'),
                     P.join(opt.destination,'src'),
                     ignore=shutil.ignore_patterns('Makefile','apps','unitTest.cpp','tsts'))
